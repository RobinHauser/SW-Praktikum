class SimilarityMeasure:
    ...
    def __get_num_sim(num1, num2):
        ...
        Then it is checked wether the difference is greater than the border and, if necessary, it gets replaced by the
    ...
    @staticmethod
    def __get_string_sim(info1, info2):
        """Get the similarity between two strings
            ...
            """
        info1 = info1.lower().split(" ")
        info2 = info2.lower().split(" ")
        similar_words = 0
        ...
        return similar_words / min(len(info1), len(info2))
    ...
    def get_similarity_measure(self):
        ...
        for prop in self.__profile1:
            if self.__profile1[prop] != "Keine Angabe" and self.__profile2[prop] != "Keine Angabe":
                if (type(self.__profile1[prop])) == str:
                    list_diff.append(self.__get_string_sim(self.__profile1[prop], self.__profile2[prop]))
                else:
                    list_diff.append(self.__get_num_sim(self.__profile1[prop], self.__profile2[prop]))
