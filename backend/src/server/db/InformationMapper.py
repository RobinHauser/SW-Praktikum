...
class InformationMapper(Mapper.Mapper):
    ...
    def find_by_id(self, id):
        result = None
        cursor = self._cnx.cursor()
        command = "SELECT * FROM information WHERE id={}".format(id)
    ...
    def insert(self, information):
        cursor = self._cnx.cursor()
        cursor.execute("SELECT MAX(id) AS maxid FROM information ")
        tuples = cursor.fetchall()

        for (maxid) in tuples:
            information.set_id(maxid[0] + 1)

        command = "INSERT INTO information (id, name, type) VALUES (%s,%s,%s)"
        data = (information.get_id(), information.get_name(), information.get_type())
    ...
    def update(self, information):
        ...
        command = "UPDATE information SET name=%s, type=%s WHERE id=%s"
    ...
    def delete(self, information):
        ...
        command = "DELETE FROM information WHERE id={}".format(information.get_id())
        cursor.execute(command)
